...
function geojsonToOsmGeometry(geom, tags, relationMembers) {
  switch (geom.type) {
    case 'Point': {
      return [new osmNode({ tags, loc: geom.coordinates })];
    }

    case 'MultiPoint': {
      const children = geom.coordinates.map((loc) => new osmNode({ loc }));
      const site = new osmRelation({
        tags: { type: 'site', ...tags },
    ...
    case 'MultiLineString': {
      ...
      for (const segment of geom.coordinates) {
        const segmentNodes = segment.map((loc) => new osmNode({ loc }));
        const way = new osmWay({ nodes: segmentNodes.map((n) => n.id) });
    ...
    case 'GeometryCollection': {
      return [new osmRelation({ tags, members: relationMembers })];
    }
    ...
    case 'MultiPolygon': {
      ...
      for (const rings of groups) {
        for (const [index, ring] of rings.entries()) {
          const ringNodes = ring.map((loc) => new osmNode({ loc }));
          ...
          const way = new osmWay({ nodes: ringNodes.map((n) => n.id) });
